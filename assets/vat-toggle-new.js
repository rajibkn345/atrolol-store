  ...
  class VatToggle {
    ...
    changeVAT(vat) {
      if (vat === "inc") {
        ...
        this.textTag.innerHTML = "inc VAT";
        ...
        this.textTag.innerHTML = "exc VAT";
    ...
    updatePriceLabels(showIncVat) {
      ...
      gpPriceElements.forEach((priceEle) => {
        ...
        try {
          ...
          if (taxable === true) {
            ...
            if (showIncVat) {
              priceEle.innerHTML = `${priceIncVat} <span class="vat-label-pp">inc VAT</span>`;
            } else {
              priceEle.innerHTML = `${priceExcVat} <span class="vat-label-pp">exc VAT</span>`;
            }
          } else {
            priceEle.innerHTML = `${priceExcVat} <span class="vat-label-pp">VAT free</span>`;
          }
        } catch (err) {
      ...
      priceElements.forEach((priceElement) => {
        ...
        if (basePrice) {
          ...
          if (isVatFree) {
            priceElement.innerHTML = `${priceExcVat} <span class="vat-label-pp">VAT free</span>`;
          } else {
            if (showIncVat) {
              const priceIncVat = (parseFloat(priceExcVat) * 1.25).toFixed(2);
              priceElement.innerHTML = `${priceIncVat} <span class="vat-label-pp">inc VAT</span>`;
            } else {
              priceElement.innerHTML = `${priceExcVat} <span class="vat-label-pp">exc VAT</span>`;
            }
          }
        }
      });
      ...
      compareElements.forEach((priceElement) => {
        ...
        if (baseComparePrice) {
          ...
          if (showIncVat) {
            const priceIncVat = (parseFloat(priceExcVat) * 1.25).toFixed(2);
            priceElement.innerHTML = `<s class="price__was">${priceIncVat}</s> <span class="vat-label-pp">inc VAT</span>`;
          } else {
            priceElement.innerHTML = `<s class="price__was">${priceExcVat}</s><span class="vat-label-pp">exc VAT</span>`;
          }
        }
      });
